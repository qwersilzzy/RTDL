...
if __name__ == "__main__":
    ...
    bins_range = [int, 10, 90, 100]
    bins_values = generate_values(bins_range)
    search_space_size =  len(bins_values)

    print('bins_values, search_space_size:::', bins_values, search_space_size)

    # directory setup
    test_dir_name = "tuned_control_AD_g3"
    # test_dir_name = "tuned_control_0"
    # test_dir_name = "tuned_control_test"
    # test_dir_name = "tuned_test_cat_1"
    original_dir_name = "tuning/0/best.toml"

    target_dir =   f"output/{datasets[10]}/{models_path_name[1]}/{test_dir_name}"
    original_dir = f"output/{datasets[10]}/{models_path_name[1]}/{original_dir_name}"
    # training model selection
    models_used = models_origin_or_MHE[5]
    ...
    create_log_txt(full_log_path, accuracy_or_rmse_all)
