...
def inverse_norm(x, scaler):
    y = torch.zeros(x.shape[0],scaler.n_features_in_).cpu()
    y[:,-1] = x
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emb_siz', type=int, default=200)
    ...
    parser.add_argument('--epoch', '-epoch', type=int, default=3, help='number of training epoch')
    parser.add_argument('--intv', '-intv', type=float, default=5)  # 单位是小时
    parser.add_argument('--bins', '-bins', type=int, default=20)
    ...
    model = house_pred(module=args.module, total=args.bins*2,
                       inv=args.intv, hidden=args.hid_siz, emb_siz=args.emb_siz).cpu()
